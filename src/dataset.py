...
class KITTI2D(Dataset):
    """
        ...
        Args
            ...
            fraction (float): 0 < fraction <= 1.0 defining the subset of images to use. (Primarily for debugging
            purposes)
            split_ratio (float): 0 < split_ratio <= 1.0 defining the amount of split for training and validation (
    ...
    def _read_label(self, index):
        """
            Read the txt file corresponding to the label and output the label tensor following the yolo format [
            max_objects x 5]
        ...
        if os.path.exists(label_filename):
            ...
            w, h, pad, padded_h, padded_w = self.state_variables["w"], self.state_variables["h"], self.state_variables[
                "pad"], self.state_variables["padded_h"], self.state_variables["padded_w"]
